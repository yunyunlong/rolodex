...
function Rolodex(config, options) {
	...
	try {
		...
		for (var routeName in config.routes) {
			...
			if (routeName === "auth" || routeName === "authCallback" || routeName === "refetch" || routeName === "token" || routeName === "logout" || routeName === "client") {
				route += "\\/(.*)";
			} else
...
Rolodex.prototype.registerRoute = function(app, routeName) {
	...
	app[(
		...
		try {
			...
			if (routeName === "token") {
				return self.services.getForSessionID(req.query.sid || req.sessionID, function(err, servicesSession) {
					if (err) return next(err);
					return res.end(servicesSession.services[serviceIdForParams(req.params)].token);
				});
			} else
			...
			if (routeName === "auth") {
				...
				function ensureFresh (callback) {
					if (req.body.fresh !== "true") {
						return callback(null);
					}
					return self.services.getForSessionID(req.sessionID, function(err, servicesSession) {
						if (err) return callback(err);
						return servicesSession.logoutAllServices(function (err) {
							if (err) return callback(err);
							return callback(null);
						});
					});
				...
				return ensureFresh(function (err) {
					if (err) return next(err);
